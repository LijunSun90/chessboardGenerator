...
void help(char* argv[]){
    printf("\n\n"
    " Calling convention:\n"
    "   chessboardGenerator blockSize board_w board_h flag_circle \n"
    "\n"
    " WHERE:\n\n"
    ...
    " flag_circle      --optional: 1: show a circle on each corner;\n"
    "                    0: Default. Withoug circles on the chessboard.\n"
    "\n"
    " Tip: \n"
...
int main(int argc, char* argv[]){
    ...
    if(argc > 4) flag_circle = atoi(argv[4]);
    ...
    // Draw circles if required by users.
    //
    cout << "image size: " << image_sz << endl;
    if (flag_circle == 1){
        for (int col = blockSize; col < image_sz.height - blockSize + 1; col = col + blockSize){
            for (int row = blockSize; row < image_sz.width - blockSize + 1; row = row + blockSize){
                // cout << cv::Point(col, row) << endl;
                cv::circle(chessboard, cv::Point(col, row), 7,  CV_RGB(255, 0, 0), 2, cv::LINE_AA);
            }
        }
    }

    cv::imshow("chessboard: Press Y or y to store this image", chessboard);
    cout << "\nPress Y or y to store this image, otherwise exit: Y or N: " << endl;
    int key = cv::waitKey(0) & 255;
    if( key == 89 || key == 121 ){ // ASCII of Y is 27, y is 121.
        // SAVE THE CHESSBOARD.
        cout << "\n\n*** DONE! ***\n" <<
